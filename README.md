...
- **新規登録** (`case.html`、idなし)：現場名・顧客名・見積もり依頼受注日・現場住所・現地写真・現地確認日候補（1〜2）・営業担当・着工予定日・完工予定日・添付資料を入力（要件定義_6の入力順に対応）。
- **詳細編集** (`case.html`、id付き)：上記に加え、顧客連絡先・対応業者名・原価・客出・粗利（自動計算）・ステータス・備考を含む全項目を表示・編集。削除も可能（確認ダイアログあり）。
...
- 粗利は原価・客出の両方が入力されているときのみ「客出−原価」で自動計算され、保存されます（片方でも未入力なら空欄）。
